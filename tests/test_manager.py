...
def test_settings(app, dynamo):
    ...
    asak_key = 'AWS_SECRET_ACCESS_KEY'
    assert app.config[asak_key] == environ.get(asak_key)
    ar_key = 'AWS_REGION'
    ar_default = Dynamo.DEFAULT_REGION
    ar_value = environ.get(ar_key) if environ.get(ar_key) else ar_default
    assert app.config[ar_key] == ar_value
...
def test_table_access(active_dynamo, app):
    with app.app_context():
        ...
        for table_name, table in active_dynamo.tables.items():
            ...
            tn = current_app.extensions['dynamo'].tables[table_name].name
